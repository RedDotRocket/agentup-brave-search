...
class TestBravePlugin:
    ...
    def test_validate_config_without_api_key(self):
        ...
        assert result["valid"] is True  # Changed expectation since API key is now optional
        assert len(result["warnings"]) > 0
        assert "No API key found" in result["warnings"][0]
    ...
    def test_validate_config_invalid_count(self):
        ...
        config = {"api_key": "test_key", "max_results": 25}
        result = self.plugin.validate_config(config)

        assert result["valid"] is False  # Should be invalid with max_results > 20
        assert len(result["errors"]) > 0

    def test_can_handle_task_search_keywords(self):
        ...
        # Remove can_handle_task test since it's not implemented in the plugin
        # This test was based on an old plugin architecture

    def test_can_handle_task_question_keywords(self):
    ...
    @patch("agentup_brave.plugin.BraveClient")
    async def test_execute_capability_success(self, mock_brave_class):
        ...
        self.mock_context.metadata = {"parameters": {"input": "python tutorials"}}

        result = await self.plugin.search_internet(self.mock_context)

        assert result["success"] is True
        assert "Web Results" in result["content"]
        assert "Test Title" in result["content"]

    async def test_execute_capability_no_api_key(self):
    ...
    async def test_execute_capability_search_error(self, mock_brave_class):
        ...
        # Set up context with parameters
        self.mock_context.metadata = {"parameters": {"input": "search for something"}}

        result = await self.plugin.search_internet(self.mock_context)

        assert result["success"] is False
        assert "Error in Brave Search" in result["content"]
    ...
    def test_get_ai_functions(self):
        ...
        assert functions[0].name == "search_internet"
        assert "input" in functions[0].parameters["required"]
...
"""
    ...
    async def test_format_search_results_empty(self):
        ...
        formatted = await self.plugin._format_search_results(mock_results)
